...
function makeRequest(port, parsedArgs, retryCount = 0) {
  return new Promise((resolve) => {
    ...
    const req = http.request(options, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          console.error(`[EnhanceChat] JSON 解析失败: ${e.message}`);
          console.error(`[EnhanceChat] 原始响应: ${body.substring(0, 200)}`);
    ...
    req.on('error', (e) => {
      console.error(`[EnhanceChat] 连接失败: ${e.message}`);

      // 如果是连接被拒绝且重试次数少于3次，则重试
      if ((e.code === 'ECONNREFUSED' || e.code === 'ECONNRESET') && retryCount < 3) {
        console.log(`[EnhanceChat] 尝试重连 (${retryCount + 1}/3)...`);
        setTimeout(() => {
...
function formatOutput(response) {
  ...
  if (!action && error) {
    return `[EnhanceChat 服务器错误] ${error}\n\n请检查插件状态后重试。${END_INSTRUCTION}`;
  }
  ...
  if (action === 'error') {
    return `[EnhanceChat 系统错误] ${error || '未知错误'}\n\n请检查插件状态后重试。${END_INSTRUCTION}`;
  }

  // 未知 action 类型 - 不应该发生
  if (!action || !['continue', 'end', 'instruction'].includes(action)) {
    return `[EnhanceChat 未知响应] action=${action}, 原始响应: ${JSON.stringify(response).substring(0, 200)}\n\n请检查插件版本是否匹配。${END_INSTRUCTION}`;
  }
...
function readPort() {
  // 优先从工作区目录查找端口文件（推荐方式）
  const cwdPortFile = path.join(process.cwd(), '.EnhanceChatopen', 'port');
  // 回退到脚本所在目录（兼容旧版本）
  ...
  if (!portFile) {
    console.error(`[EnhanceChat] 错误: 端口文件不存在`);
    console.error(`[EnhanceChat] 已检查路径:`);
    console.error(`  - ${cwdPortFile}`);
    console.error(`  - ${localPortFile}`);
    console.error(`[EnhanceChat] 请确保 EnhanceChatOpen 插件已启动，且当前工作目录正确`);
    console.error(`[EnhanceChat] 当前工作目录: ${process.cwd()}`);
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(`[EnhanceChat] 错误: 无效的端口号: ${portStr}`);
    process.exit(1);
  } catch (e) {
    console.error(`[EnhanceChat] 读取端口文件失败: ${e.message}`);
    process.exit(1);
  }
}
